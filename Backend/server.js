...
app.post('/api/applications', upload.fields([
    ...
    try {
        ...
        const result = await pool.query(
            'INSERT INTO applications (personal_info, education, work_experience, documents, date, status, offer_letter, offer_letter_date) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id',
            [formData.personalInfo, formData.education, formData.workExperience, formData.documents, formData.date, formData.status, formData.offer_letter, formData.offer_letter_date]
        );
...
app.get('/api/offer-letters', async (req, res) => {
    try {
        ...
        const offset = (page - 1) * limit;
        let query = 'SELECT * FROM applications WHERE offer_letter IS NOT NULL';
        const queryParams = [];

        if (search) {
            query += ` AND (personal_info->>'name' ILIKE $${queryParams.length + 1} OR ` +
                     `personal_info->>'email' ILIKE $${queryParams.length + 1} OR ` +
                     `personal_info->>'phone' ILIKE $${queryParams.length + 1})`;
            queryParams.push(`%${search}%`);
        }

        query += ' ORDER BY offer_letter_date DESC LIMIT $' + (queryParams.length + 1) + ' OFFSET $' + (queryParams.length + 2);
        queryParams.push(limit, offset);

        const result = await pool.query(query, queryParams);
        const totalResult = await pool.query(
            'SELECT COUNT(*) FROM applications WHERE offer_letter IS NOT NULL' + (search ? ' AND (personal_info->>\'name\' ILIKE $1 OR personal_info->>\'email\' ILIKE $1 OR personal_info->>\'phone\' ILIKE $1)' : ''),
            search ? [`%${search}%`] : []
        );

        res.json({ offerLetters: result.rows, total: parseInt(totalResult.rows[0].count) });
    } catch (error) {
        console.error('Error fetching offer letters:', error);
        res.status(500).json({ error: 'Failed to fetch offer letters: ' + error.message });
    }
});

// Get offer letter by name and email
app.get('/api/offer-letter', async (req, res) => {
    try {
        ...
        const result = await pool.query(
            'SELECT personal_info, offer_letter, offer_letter_date FROM applications WHERE offer_letter IS NOT NULL AND personal_info->>\'name\' = $1 AND LOWER(personal_info->>\'email\') = LOWER($2)',
            [name, email]
        );

        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'No offer letter found for the provided name and email' });
        }

        res.json(result.rows[0]);
    } catch (error) {
        console.error('Error fetching offer letter:', error);
        res.status(500).json({ error: 'Failed to fetch offer letter: ' + error.message });
    }
});
...
// Store offer letter
app.patch('/api/applications/:id/offer-letter', async (req, res) => {
    const { id } = req.params;
    const { offer_letter } = req.body;

    if (!offer_letter || typeof offer_letter !== 'string' || offer_letter.trim() === '') {
        return res.status(400).json({ error: 'Invalid offer letter content' });
    }

    try {
        const result = await pool.query(
            'UPDATE applications SET offer_letter = $1, offer_letter_date = $2 WHERE id = $3 RETURNING *',
            [offer_letter, new Date().toISOString(), id]
        );
        ...
        res.json({ message: 'Offer letter stored successfully', application: result.rows[0] });
    } catch (error) {
        console.error('Error storing offer letter:', error);
        res.status(500).json({ error: 'Failed to store offer letter: ' + error.message });
    }
});
